...
function getUrl({ url, method, data, version = "v1" }) {
  ...
  let convertedUrl =
    apiStringIndex > 3
      ? `${url.substring(0, apiStringIndex)}/${version}${url.substring(apiStringIndex)}`
      : `/api/${version}${url.startsWith("/") ? url : "/" + url}`;
  if (data && data.pathVariable)
    convertedUrl = convertedUrl.replace(/{(.*)}/g, (a, g1) => {
      return data[g1];
    });
  return isGet(method) && data && !data.pathVariable ? getUrlWithData(convertedUrl, data) : convertedUrl;
}
